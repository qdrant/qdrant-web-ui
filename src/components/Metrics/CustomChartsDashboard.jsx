...
// NOTE: This is the earlier user-built "custom charts" dashboard — a metric
// search bar, presets, and per-chart series editing, persisted to localStorage.
// It's kept in the repo but is no longer rendered on the Metrics page, which now
// shows a fixed set of auto-created preset panels (see MetricsDashboard.jsx).
...
function CustomChartsDashboard({ embedded = false }) {
  ...
  return (
    <Box>
      {/* Page header — hidden when embedded under another dashboard. */}
      {!embedded && (
...
CustomChartsDashboard.propTypes = {
  // When true, the component's own page header is hidden so it can be embedded
  // beneath another dashboard.
...
export default CustomChartsDashboard;
