...
The first step is to create a collection that can handle sparse vectors. In this example, we’ll create a collection called `sparse_charts` with a vector size of 4 and the Cosine distance metric for similarity calculation. The sparse vectors will be indexed in memory (no on-disk storage).
